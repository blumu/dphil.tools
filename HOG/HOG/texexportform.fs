...
let lnfrules_to_latexcompgraph (lnfrules:lnfrule list) =
    ...
    let index_of_nt name = Array.findIndex (function (n,_) -> name = n)  ar_lnfrules in
    ...
    let pst_node_var x = "\\TR{"^(format_var x)^"}" in
    let pst_node_const c = "\\TR{\\framebox{"^c^"}}" in
    ...
    let rec build_nt_subgraph incomingedge_label i_nt =
        ...
        let optlambda = if nt_occs.(i_nt) > 1 then "[name="^(string_of_int i_nt)^"]" else "" in
        let edgelabel = if incomingedge_label = "" then "" else "\\ncput*{\\scriptstyle "^incomingedge_label^"}" in
