(** Description: View used for Higher-order recursion schemes
    Author: William Blum
**)
module View.HigherOrderRecursionScheme
...
/// The form used to show a higher-order recursion scheme.
type Form = 
  class
    ...
    member this.validate_valuetree_path (node:TreeNode) =
      if node.Tag <> null then
          let nodepath = View.Cpda.TreeNode_get_path (if is_terminal_treeviewnode node then node else node.Parent)
    ...
    member this.InitializeComponent() =                
        ...
        this.btRun.Click.Add(fun _ -> let node = this.valueTreeView.SelectedNode
                                      if is_expandable_treeviewnode node then
                                         while not (expand_term_in_treeview this.hors node) do
                                            ()
        ...
        this.btCpda.Click.Add(fun _ -> //create the cpda form
                                        let form = new View.Cpda.Form("CPDA built from the recursion scheme "^this.filename,
        ...
        this.btPda.Click.Add(fun _ -> //create the pda
                                            let form = new View.Cpda.Form("PDA built from the recursion scheme "^this.filename,
        ...
        this.btNp1pda.Click.Add(fun _ -> //create the pda
                                            let form = new View.Cpda.Form("n+1-PDA built from the recursion scheme "^this.filename,
