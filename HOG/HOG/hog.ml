...
(** Alphabet of a higher-order grammar *)
type alphabet = (ident * typ) list;;

(** an applicative term (right-hand side of a grammar rule) *)
...
type recscheme = {
    ...
    (** The path validator is a function that checks whether a given path (list of terminals)
      generated by the recscheme satisfies some desired specification. It returns a pair [(v,c)] where [v] tells
...
let terminal_type rs f =
    match List.try_assoc f rs.sigma with
    | Some v -> v
    | None -> raise (Wrong_terminal_name f)
;;

(* return the type of a non-terminal *)
let nonterminal_type rs nt =
  match List.try_assoc nt rs.nonterminals with
  | Some v -> v
  | None -> raise (Wrong_nonterminal_name nt)
;;
    ...
    let _,parms,rhs = List.find (function rname,_,_ -> rname=nt) rs.rules in
      ...
      if List.length parms = List.length operands then
        ...
        let rec subst term =
          ...
          | Var(x) ->
            match List.try_assoc x substlst with
            | Some vx -> vx
            | None -> term
        in
          subst rhs
...
let get_parameter_type rs x =
  ...
  match List.tryFind (function (_,parms,_) -> List.mem x parms) rs.rules with
  | Some (nt,parms,_) ->
  ...
  | None ->
    failwith ("get_parameter_type: formal parameter '"^x^"' not used in the recursion scheme.")
;;
...
let rule_to_lnf rs (nt,param,rhs) =
  ...
  and lnf_aux typ appterm =
    ...
    let absvars_types = create_param_typ_list absvars typ in
    (* compute the lnfs of the operands *)
    let lnfoperands = List.map lnf operands in
  ...
  in
    (nt, (param, (snd (lnf rhs)))),(* the first component of the pair returned by 'lnf rhs'
