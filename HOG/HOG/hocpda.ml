...
let hocpda_label_to_address cpda lab =
    match List.try_assoc lab cpda.labels with
    | Some v -> v
    | None -> raise (Cpda_execution_failed("(label referencing)","The code of the HOCPDA contains references to undefined labels!"))
;;
...
let rec execute cpda ip stk =
    ...
    | CaseTop0(cases) -> (let top0elem = fst (top0 cpda.n stk) in
                          match list_tryfind (function stkelems, _ -> List.mem top0elem stkelems) cases with
                          | Some (_, lab) ->
                                (mk_state cpda (hocpda_label_to_address cpda lab)),stk
                          | None ->
                                (mk_nextstate cpda ip),stk
                         )
    | CaseTop0Do(cases) -> (let top0elem = fst (top0 cpda.n stk) in
                            match list_tryfind (function stkelems, _ -> List.mem top0elem stkelems) cases with
                            | Some (_, ins) ->
                                execute cpda ip stk ins
                            | None -> (mk_nextstate cpda ip),stk)
...
let hlicode_to_cpdacode hlicode =
    ...
    let ip_from_label label =
        match List.try_assoc label label2ip with
        | Some v -> v
        | None -> failwith "The high-level source code contains references to undefined labels!"
    in
